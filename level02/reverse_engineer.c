...
// We can see that override is in 64 bits because of register names and addresses length
int main()
{
    // alloc stack for 0x120 bytes (288)

    // save edi to rbp-0x114
    // save rsp to rbp-0x120

    void *handle = fopen("/home/users/level03/.pass", "r");

    // happens in gdb
    if (handle == NULL)
    {
        fwrite("ERROR: failed to open password file", 1, 33, stderr);
        exit(1);
    }

    char user_name[0x64] = {0}; // rbp-0x70  (Pretty sure about size but not 100%)
    char real_pass[0x30] = {0}; // rbp-0xa0  (Pretty sure about size but not 100%)
    char user_pass[0x64] = {0}; // rbp-0x110 (Pretty sure about size but not 100%)

    int read = fread(real_pass, 1, 0x29, handle);
    real_pass[strcspn(real_pass, "\n")] = 0;

    if (read != 0x29)
    {
        fwrite("ERROR: failed to read password file", 1, 35, stderr);
        exit(1);
    }
    ...
    printf("--[ Username: ");
    fgets(user_name, 0x64, stdin);
    user_name[strcspn(user_name, "\n")] = 0;

    printf("--[ Password: ");
    fgets(user_pass, 0x64, stdin);
    user_pass[strncmp(real_pass, user_name, 0x29)] = 0;

    puts("*****************************************");

    if (strncmp(real_pass, user_pass, 0x29) == 0)
    {
        printf("Greetings, %s!\n", user_name);
        system("/bin/sh");
        return 0;
